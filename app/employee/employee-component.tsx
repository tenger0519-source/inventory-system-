    ...
    <div className="overflow-auto">
      <div className="min-w-[600px]">
        <div className="grid grid-cols-8 border-b">
          <div className="p-2 text-muted-foreground">Öàã</div>
                    ...
                    <div 
                      ...
                      onClick={(e) => {
                        ...
                        if (isTaskStart(dayIndex, 8 + hour, task)) {
                          alert(`Äààëãàâàð: ${task.action}\nÒºðºë: ${task.type}\nÒºëºâ: ${task.completed ? 'Ã³éöýòãýñýí' : 'Õ³ëýýäæ áàéíà'}\n${task.message || ''}`)
                        }
                      }}
                    >
            ...
            {tasksList.length > 0 ? tasksList.map((task: any, i: number) => (
              <div key={i} className={`rounded-lg border p-4 space-y-2 ${task.completed ? 'bg-muted/50 opacity-60' : ''}`}>
                <div className="flex justify-between items-start">
                  ...
                  {task.completed && (
                    <span className="ml-3 text-sm text-green-600 font-medium">Ã³éöýòãýñýí</span>
                  )}
                </div>
                {task.type === "move" && (
                  <>
                    {task.quantity > 0 && (
                      <p className="text-sm text-muted-foreground">{task.product} â {task.quantity}ø</p>
                    )}
                    <p className="text-sm font-medium">{task.from} â {task.to}</p>
                    <p className="text-sm text-muted-foreground">Òàâèóð: {task.shelf}</p>
                  </>
                )}
                {task.type === "message" && (
